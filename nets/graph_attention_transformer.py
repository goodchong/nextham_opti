...
from tg_src.model import EquiConv, EdgeUpdateBlock, NodeUpdateBlock
from tg_src.e3modules import e3ElementWise
...
# added by fywang
# direct_product_expansion_Taylor
def compute_clebsch_gordan_matrix(l1, l2):
    CG = torch.cat([wigner_3j(l, l1, l2) for l in range(abs(l2 - l1), abs(l2 + l1) + 1)], dim=0)
    return CG

def equi_lm(f):
    mu_c = torch.mean(f, dim=-1, keepdim=True)
    sigma = torch.mean((f-mu_c)**2, dim=(-1,-2), keepdim=True).sqrt()
    f_new = (f-mu_c)/(sigma+1e-6)
    return f_new

def taylor_dpe(f, l_max, do_equi_lm=True):
    '''
    f: [batch_size, 0+1+...+2l_max-1, channel_num]
    '''
    B, _, C = f.shape
    if do_equi_lm:
        f = equi_lm(f)
    ...
    out = f/2.0
    taylor_coff_silu = [1.0/4.0, 0.0, -1/48.0, 0.0, 1.0/480.0]
    for o_idx in range(len(taylor_coff_silu)):
        index = 0
        for l in range(l_max + 1):
            dim_l = 2 * l + 1
            f_l = f[:, index:index + dim_l, :]
            if o_idx==0:
                f_l_r = f_l
            else:
                f_l_r = f_prime[:, index:index + dim_l, :]
            f_l_outer = torch.einsum('bic,bjc->bijc', f_l, f_l_r).reshape(B, -1, C)            
            CG_matrix = compute_clebsch_gordan_matrix(l, l).reshape(((2*l + 1)**2, (2*l + 1)**2))
            f_l_decomposed = torch.einsum('bij,ik->bkj', f_l_outer, CG_matrix.T)
            f_l_prime = f_l_decomposed[:, l**2:l**2+dim_l, :]
            f_prime[:, index:index + dim_l, :] = f_l_prime
            index += dim_l
        out += taylor_coff_silu[o_idx]*f_prime
    return out
...
class SeparableFCTP(torch.nn.Module):
    ...
    def __init__(self, irreps_node_input, irreps_edge_attr, irreps_node_output, 
        ...
        # expanded_irreps = []
        # for mul, ir in self.irreps_node_input:
        #     expanded_irreps.extend([(1, ir)] * mul)

        # # 创建新的 Irreps 对象
        # self.irreps_node_input = o3.Irreps(expanded_irreps)
...
class GraphAttention(torch.nn.Module):
    ...
    def __init__(self,
        ...
        irreps_edge_attr, irreps_edge_sh, irreps_node_output, irreps_edge_output,
        fc_neurons,
        ...
        self.pre_lin = LinearRS(o3.Irreps(f"{self.irreps_pre_attn}+{self.irreps_pre_attn}+{self.irreps_edge_attr}"), self.irreps_pre_attn)
        ...
        self.element_wise = e3ElementWise(self.irreps_head)
        self.lin_alpha = torch.nn.Sequential(
            torch.nn.Linear(64 + mul_alpha_head, 64),
        ...
        if self.nonlinear_message:
            ...
            self.sep_act = SeparableFCTP(self.irreps_pre_attn, 
                self.irreps_edge_sh, self.irreps_pre_attn, fc_neurons, 
            ...
            self.sep_value = SeparableFCTP(self.irreps_pre_attn, 
                self.irreps_edge_sh, irreps_attn_heads, fc_neurons=None, 
            ...
            self.sep = SeparableFCTP(self.irreps_pre_attn, 
                self.irreps_edge_sh, irreps_attn_all, fc_neurons, 
        ...
        self.edge_proj = LinearRS(irreps_attn_heads, self.irreps_edge_output)
    ...
    def forward(self, node_input, node_attr, edge_src, edge_dst, edge_attr, edge_sh, edge_scalars, 
        batch, **kwargs):
        ...
        message = torch.cat((message_src[edge_src], message_dst[edge_dst], edge_attr), dim = -1)
        ...
        if self.nonlinear_message:          
            ...
            message = self.sep_act.dtp(message, edge_sh, weight)
            alpha = self.sep_alpha(message)
            alpha = self.vec2heads_alpha(alpha)
            value = self.sep_act.lin(message)
            value = self.sep_act.gate(value)
            value = self.sep_value(value, edge_attr=edge_sh, edge_scalars=edge_scalars)
            value = self.vec2heads_value(value)
        else:
            message = self.sep(message, edge_attr=edge_sh, edge_scalars=edge_scalars)
        ...
        edge_scalars = torch.cat((edge_scalars, alpha[:, 0]), dim = -1)
        alpha = self.lin_alpha(edge_scalars)
        edge_fea = value * alpha.unsqueeze(-1)
        alpha = torch_geometric.utils.softmax(alpha, edge_dst)
        alpha = alpha.unsqueeze(-1)
        if self.alpha_dropout is not None:
            alpha = self.alpha_dropout(alpha)
        attn = value * alpha # 32x0e+16x1e+8x2e, num_heads=4
        
        #edge_output = self.edge_proj(self.heads2vec_edge(attn))
        attn = scatter(attn, index=edge_dst, dim=0, dim_size=node_input.shape[0]) #
        attn = self.heads2vec(attn)
        edge_fea = self.heads2vec_edge(edge_fea)
        ...
        edge_output = self.edge_proj(edge_fea)
...
class Update_edge(torch.nn.Module):
    def __init__(self, irreps_node_input, irreps_edge_input, 
                 irreps_edge_output, fc_len_in, norm_layer='layer'):
        super().__init__()
        self.irreps_node_input = o3.Irreps(irreps_node_input)
        self.irreps_edge_input = o3.Irreps(irreps_edge_input)
        self.irreps_edge_output = o3.Irreps(irreps_edge_output)
        self.irreps_edge_outsimple = self.irreps_edge_output.sort()[0].simplify()
        self.lin = LinearRS(self.irreps_edge_outsimple, self.irreps_edge_output)
        self.irreps_edge_mid = o3.Irreps(f'{self.irreps_node_input}+{self.irreps_node_input}+{self.irreps_edge_input}')

    def forward(self, node_input, edge_src, edge_dst, edge_attr, edge_sh, edge_scalars=None, 
        batch=None, **kwargs):
...
class TransBlock(torch.nn.Module):
    ...
    def __init__(self,
        irreps_node_input, irreps_node_attr,
        irreps_edge_attr, irreps_node_output, irreps_edge_output,
        fc_neurons,
        irreps_head, num_heads, irreps_pre_attn=None, 
        rescale_degree=False, nonlinear_message=False,
        alpha_drop=0.1, proj_drop=0.1,
        ...
        self.norm_edge = get_norm_layer(norm_layer)(self.irreps_edge_attr)
        self.ga = GraphAttention(irreps_node_input=self.irreps_node_input, 
            irreps_node_attr=self.irreps_node_attr,
            irreps_edge_attr=self.irreps_edge_attr,
            irreps_edge_sh='1x0e+1x1o+1x2e+1x3o+1x4e+1x5o',
            irreps_node_output=self.irreps_node_input,
            irreps_edge_output=self.irreps_edge_output,
        ...
        self.ga_edge = GraphAttention(irreps_node_input=self.irreps_node_input, 
            irreps_node_attr=self.irreps_node_attr,
            irreps_edge_attr=self.irreps_edge_attr,
            irreps_edge_sh='1x0e+1x1o+1x2e+1x3o+1x4e+1x5o',
            irreps_node_output=self.irreps_node_input,
        ...
        # self.ffn_edge = FeedForwardNetwork(
        ...
        #     irreps_mlp_mid=self.irreps_node_input,
        #     proj_drop=proj_drop)
        self.update_edge = Update_edge(
            irreps_node_input=self.irreps_node_input,
            irreps_edge_input=self.irreps_edge_attr,
            irreps_edge_output=self.irreps_edge_output,
            fc_len_in = 32
            )
        
        self.node_update = NodeUpdateBlock(num_species=1, fc_len_in=32, irreps_sh='1x0e+1x1o+1x2e+1x3o+1x4e+1x5o', 
                                        irreps_in_node=self.irreps_node_input, 
                                        irreps_out_node=self.irreps_node_input, irreps_in_edge=self.irreps_edge_attr,
                                        act = {1: torch.nn.functional.silu, -1: torch.tanh},
                                        act_gates = {1: torch.sigmoid, -1: torch.tanh},
                                        use_selftp=False, use_sc=False, concat=True, only_ij=False, nonlin=False, norm='e3LayerNorm', if_sort_irreps=False)
        
        self.edge_update = EdgeUpdateBlock(
                        num_species=1, fc_len_in=32, irreps_sh='1x0e+1x1o+1x2e+1x3o+1x4e+1x5o', 
                        irreps_in_node=self.irreps_node_input, irreps_in_edge=self.irreps_edge_attr, 
                        irreps_out_edge=self.irreps_edge_output,
                        act = {1: torch.nn.functional.silu, -1: torch.tanh},
                        act_gates = {1: torch.sigmoid, -1: torch.tanh},
                        use_selftp=False, use_sc=False, init_edge=False, nonlin=False, norm='e3LayerNorm', if_sort_irreps=False)
    ...
    def forward(self, node_input, node_attr, edge_src, edge_dst, edge_attr, edge_sh, edge_scalars, 
        batch, edge_length, **kwargs):
        
        node_output = node_input
        edge_output = edge_attr
        ...
        node_features, edge_attr = self.ga(node_input=node_features, 
            node_attr=node_attr, 
            edge_src=edge_src, edge_dst=edge_dst, 
            edge_attr=edge_attr, edge_sh=edge_sh, edge_scalars=edge_scalars,
            batch=batch)

        if node_features_in.shape[-1] == node_features.shape[-1]:
            node_features = node_features + node_features_in

        node_features = self.norm_2(node_features, batch=batch)

        node_output = self.ffn(node_features, node_attr)
        if self.ffn_shortcut is not None:
            node_output = self.ffn_shortcut(node_output, node_attr)
        
        if node_output.shape[-1] == node_features.shape[-1]:
